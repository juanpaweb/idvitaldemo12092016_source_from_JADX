...
final class DecodedBitStreamParser {
    ...
    static /* synthetic */ class C02261 {
        static final /* synthetic */ int[] f7xb73eb560;

        static {
            f7xb73eb560 = new int[Mode.values().length];
            try {
                f7xb73eb560[Mode.C40_ENCODE.ordinal()] = 1;
            } catch (NoSuchFieldError e) {
            }
            try {
                f7xb73eb560[Mode.TEXT_ENCODE.ordinal()] = 2;
            } catch (NoSuchFieldError e2) {
            }
            try {
                f7xb73eb560[Mode.ANSIX12_ENCODE.ordinal()] = 3;
            } catch (NoSuchFieldError e3) {
            }
            try {
                f7xb73eb560[Mode.EDIFACT_ENCODE.ordinal()] = 4;
            } catch (NoSuchFieldError e4) {
            }
            try {
                f7xb73eb560[Mode.BASE256_ENCODE.ordinal()] = 5;
    ...
    static DecoderResult decode(byte[] bytes) throws FormatException {
        ...
        do {
            if (mode == Mode.ASCII_ENCODE) {
                ...
                switch (C02261.f7xb73eb560[mode.ordinal()]) {
    ...
    private static Mode decodeAsciiSegment(BitSource bits, StringBuilder result, StringBuilder resultTrailer) throws FormatException {
        ...
        do {
            ...
            if (oneByte == 0) {
                throw FormatException.getFormatInstance();
            } else if (oneByte <= TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                if (upperShift) {
                    oneByte += TransportMediator.FLAG_KEY_MEDIA_NEXT;
                }
                    ...
                    switch (shift) {
                        case ItemTouchHelper.ACTION_STATE_IDLE /*0*/:
                            if (cValue < 3) {
                                ...
                                result.append((char) (c40char + TransportMediator.FLAG_KEY_MEDIA_NEXT));
                                upperShift = false;
                                break;
                            } else {
                                throw FormatException.getFormatInstance();
                        ...
                        case ItemTouchHelper.UP /*1*/:
                            if (upperShift) {
                                result.append((char) (cValue + TransportMediator.FLAG_KEY_MEDIA_NEXT));
                                upperShift = false;
                            } else {
                    ...
                    switch (shift) {
                        case ItemTouchHelper.ACTION_STATE_IDLE /*0*/:
                            if (cValue < 3) {
                                ...
                                result.append((char) (textChar + TransportMediator.FLAG_KEY_MEDIA_NEXT));
                                upperShift = false;
                                break;
                            } else {
                                throw FormatException.getFormatInstance();

...
public final class Code93Reader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> map) throws NotFoundException, ChecksumException, FormatException {
        ...
        if (nextStart == end || !row.get(nextStart)) {
            ...
            r19 = new ResultPoint[2];
            r19[0] = new ResultPoint(((float) (start[1] + start[0])) / 2.0f, (float) rowNumber);
            r19[1] = new ResultPoint(right, (float) rowNumber);
            return new Result(resultString, null, r19, BarcodeFormat.CODE_93);
        }
    }
            ...
            if (c < 'a' || c > 'd') {
                ...
                switch (c) {
                    case C0239R.styleable.AppCompatTheme_textColorAlertDialogListItem /*97*/:
                        ...
                        throw FormatException.getFormatInstance();
                        break;
                    case C0239R.styleable.AppCompatTheme_buttonBarPositiveButtonStyle /*98*/:
                        if (next < 'A' || next > 'E') {
                            if (next < 'F' || next > 'J') {
                                if (next < 'K' || next > 'O') {
                                    if (next < 'P' || next > 'S') {
                    ...
                    case C0239R.styleable.AppCompatTheme_buttonBarNeutralButtonStyle /*100*/:
                        if (next >= 'A' && next <= 'Z') {
                            decodedChar = (char) (next + 32);
                            break;
                        }
